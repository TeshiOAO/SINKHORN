...
def train(cfg):
    ...
    torch.save(model.state_dict(), os.path.join(cfg.out_dir, "model_weight"+str(cfg.n_samples)+".pth"))
...
def evaluation(cfg):
    ...
    if torch.cuda.is_available():
        ...
        device = "cpu"
    model = FCModel(cfg.hid_c, cfg.n_numbers).to(device)
    model.load_state_dict(torch.load(os.path.join(cfg.out_dir, "model_weight"+str(cfg.n_samples)+".pth")))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tau", default=5.0, type=float, help="temperture parameter")
    parser.add_argument("--n_sink_iter", default=20, type=int, help="number of iterations for sinkhorn normalization")
    parser.add_argument("--n_samples", default=7, type=int, help="number of samples from gumbel-sinkhorn distribution")
    # datase option
